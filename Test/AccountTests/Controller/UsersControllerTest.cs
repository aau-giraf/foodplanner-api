...
public class UsersControllerTests
{
    ...
    [Fact]
    public async Task GetLoggedIn_ValidToken_ReturnsUserData()
    {
        //Arrange
        var mockUserService = new Mock<IUserService>();
        var authService = new Mock<IAuthService>();
        ...
        authService.Setup(auth => auth.RetrieveIdFromJwtToken(validToken)).Returns(userId.ToString());
        ...
        mockUserService.Setup(s => s.GetLoggedInUserAsync(userId)).ReturnsAsync(expectedUserDTO);

        var controller = new UsersController(mockUserService.Object, authService.Object);

        // Act
        var result = await controller.GetLoggedIn($"Bearer {validToken}");
    ...
    [Fact]
    public async Task UpdateLoggedIn_ValidTokenAndValidData_ReturnsCreated()
    {
        // Arrange
        var token = "valid-jwt-token";
        var userId = 1;
        ...
        mockUserService.Setup(service => service.UpdateUserLoggedInAsync(userId, userUpdateDTO)).ReturnsAsync(1);

        var controller = new UsersController(mockUserService.Object, mockAuthService.Object);
        //Act
        var result = await controller.UpdateLoggedIn($"Bearer {token}", userUpdateDTO);
    ...
    [Fact]
    public async Task UpdatePassword_ValidTokenAndValidPassword_ReturnsCreated()
    {
        // Arrange
        var token = "valid-jwt-token";
        var userId = 1;
        var newPassword = new Password { password = "NewPassword123" };
        ...
        mockUserService.Setup(service => service.UpdateUserPasswordAsync(newPassword.password, userId)).ReturnsAsync(1);

        var controller = new UsersController(mockUserService.Object, mockAuthService.Object);

        // Act
        var result = await controller.UpdatePassword($"Bearer {token}", newPassword);

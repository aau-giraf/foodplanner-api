    ...
    */
    public class PackedIngredientRepository (PostgreSQLConnectionFactory connectionFactory) : IPackedIngredientRepository
    {
        ...
        public async Task<int> InsertAsync(PackedIngredient entity) {
            ...
            // SQL query to insert a new packed ingredient, dynamically generating columns and values
            var sql = $"INSERT INTO packed_ingredients ({string.Join(", ", GetContent(entity))})\n"; 
            sql += $"VALUES ({string.Join(", ", GetContent(entity, false))})";

            // Executes the insert query and returns the number of affected rows
